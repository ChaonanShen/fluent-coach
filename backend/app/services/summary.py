...
from backend.app.models import GrammarSeverity, Scenario, Session, SessionSummary, TurnSpeaker
...
class SummaryService:
    def summarize(self, *, session: Session, scenario: Scenario) -> SessionSummary:
        ...
        top_issues = self._top_issues(issues)
    ...
    def _next_drills(self, *, top_issues: list[str], scenario: Scenario) -> list[str]:
        if top_issues:
            return [f"Review {issue.replace('_', ' ')} in one short answer." for issue in top_issues[:3]]
