...
`Conversation Assessment` 展示当前对话中的核心反馈，按用户每一轮输入逐条排列。

每条反馈可能包含：

- `Overall`、`Accuracy`、`Fluency`：本轮语音发音评测分数；文字输入没有发音结果时不会显示这一行。
- `Original`：用户原话。语音回合里，低分单词会直接在原句中高亮。
- `Corrected`：系统修正后的版本。
- 中文解释：说明本轮主要的语法、表达或自然度问题。
- `Issues`：ASR、语法分析或发音评测失败时，会展示可读的错误提示。

多轮对话会保留历史反馈，区域固定高度，默认滚动到最新一条。语法和表达在数据层是分开的：语法问题来自具体错误点，表达问题通常是“这句话可以更自然、更职业、更贴合场景”。发音反馈只在语音回合或独立朗读练习中出现。
...
`Summary` 是当前会话结束后的整体表现概览，当前展示这些指标：
...
- `Conversation Assessment`：按轮展示语法/表达修正、发音分数和本轮分析错误。
- `Reading Practice`：独立朗读练习的转写和发音评测结果。
- `Timing`：性能耗时，不是语言错误。
